---
title: "Cours_Modelisation_Ecologie"
...
# **Script R du cours "Modélisation en écologie"** 

Le plan du script est le suivant

* **Simulation d'une croissance exponentielle avec différentes valeurs de lamdba**: 
  - lambda = 1.0355
  - Lambda = 0.8
  - Lambda = 1

* **Analyses de données réelles : comptage de populations de grizzly dans le Parc National du Yellowstone**:
  - *Chargement des données et analyses*:
    - Estimation du taux de multiplicationet de la variance
    
  - *Comparaison données réelles vs modèle déterministe*:
    - Production de la figure 
 
* **Prise en compte de la stochasticité environnementale**:
~ Simulations de projections de populations avec prise en compte de stochasticité 
- Production de la figure 
...
nyears <- 30                # Nombre d'années à projeter
...
plot(N1~year, col = "#440154FF", pch = 19, ylim = c(0,1500))   # Production de la première figure
...
year <- seq(from=1959, to=1997, by=1)    # Séquence des années
...
n0 <- grizzly$N[1] # Population initiale en 1959
n0
